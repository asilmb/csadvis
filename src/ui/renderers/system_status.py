...
def render_system_status(health=None) -> html.Div:
    ...
    btn_row = html.Div([
        dbc.Row([
            ...
            dbc.Col(html.Div([
                ...
                _btn_group(
                    ...
                    dbc.Button("Refresh Portfolio", id="btn-refresh-portfolio", color="secondary", outline=True, size="sm", n_clicks=0),
                ),
        ...
        dbc.Tooltip("Пересчитать только портфель (allocate_portfolio) без пересчёта сигналов — быстрее чем Refresh Cache", target="btn-refresh-portfolio", placement="bottom"),
