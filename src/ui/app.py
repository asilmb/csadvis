...
_VERSION = "2.3.8"  # fallback; real value served from Redis
...
class _CachedVersion:
    ...
    def __init__(self, value: str, expire_at: float) -> None:
        self.value = value
        self.expire_at = expire_at


_ver_cache = _CachedVersion(value=_VERSION, expire_at=0.0)


def get_dynamic_version() -> str:
    global _ver_cache
    now = time.time()
    if now < _ver_cache.expire_at:
        return _ver_cache.value
    try:
        raw = get_redis().get(_VERSION_REDIS_KEY)
        v = (raw.decode() if isinstance(raw, bytes) else str(raw)) if raw else _VERSION
    except Exception as exc:
        logger.warning("get_dynamic_version: Redis error — %s", exc)
        v = _VERSION
    _ver_cache = _CachedVersion(value=v, expire_at=now + _CACHE_TTL)
    return v
