...
class TestGetBalanceData:
    ...
    def test_empty_inventory_no_snapshots(self):
        with (
            patch("src.domain.portfolio.get_snapshots", return_value=[]),
            patch("src.domain.portfolio.get_portfolio_data", return_value={}),
        ):
    ...
    def test_inventory_value_calculated_from_current_prices(self):
        ...
        with (
            patch("src.domain.portfolio.get_snapshots", return_value=[]),
            patch("src.domain.portfolio.get_portfolio_data", return_value=prices),
        ):
    ...
    def test_delta_kzt_calculated_vs_oldest_snapshot(self):
        ...
        with (
            patch("src.domain.portfolio.get_snapshots", return_value=snapshots),
            patch("src.domain.portfolio.get_portfolio_data", return_value=prices),
        ):
            result = get_balance_data(1000.0, inventory)
...
class TestGetTransactionsSchema:
    ...
    def test_returns_list(self):
        ...
        with patch("src.domain.portfolio.SessionLocal", return_value=mock_session):
...
class TestDbUnavailability:
    ...
    def test_get_snapshots_raises_on_db_error(self):
        ...
        with (
            patch("src.domain.portfolio.SessionLocal", side_effect=_SA_ERROR),
    ...
    def test_get_balance_data_propagates_db_error(self):
        ...
        with (
            patch("src.domain.portfolio.get_portfolio_data", return_value={}),
            patch("src.domain.portfolio.get_snapshots", side_effect=_SA_ERROR),
            pytest.raises(SAOperationalError),
        ):
